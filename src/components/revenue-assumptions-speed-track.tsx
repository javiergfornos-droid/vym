...
export function RevenueAssumptionsSpeedTrack({ lang }: Props) {
  ...
  const revenueProjectionYear5 = useMemo(() => {
    if (revenueMode === 'initial-plus-growth') return baseRevenue * (1 + revenueGrowth / 100) ** 5 + additionalRevenueAmount;
    if (revenueMode === 'year1-year5') return year5RevenueTarget + additionalRevenueAmount;
    return constantRevenue + additionalRevenueAmount;
      ...
      <div className="space-y-5">
        <SectionCard title={t.revenueTitle}>
          ...
          <div className="grid gap-2 md:grid-cols-3">
            {[
              ...
              <button
                ...
                key={copy}
                onClick={() => {
                  if (index === 0) setRevenueMode('initial-plus-growth');
                  else if (index === 1) setRevenueMode('year1-year5');
                  else if (index === 2) setRevenueMode('constant-5y');
                  else {
                    setRevenueMode('initial-plus-growth');
                    setRevenueGrowth(BENCHMARK_RATE);
                  }
                }}
                type="button"
              >
          ...
          <div className="rounded-xl border border-line bg-white p-3">
            <p className="font-editorial text-base text-slateInk">{t.additionalRevenueTitle}</p>
            <p className="mt-1 text-sm text-mutedInk">{t.additionalRevenueSupport}</p>
            <label className="mt-3 block font-editorial text-sm text-mutedInk">
              <input
          ...
          <SummaryGrid
            assumption={
              <>
                ...
                <p>{`additional revenue amount (constant 5 years): ${formatMoney(additionalRevenueAmount, lang)}`}</p>
              </>
            }
            benchmark={
              <>
